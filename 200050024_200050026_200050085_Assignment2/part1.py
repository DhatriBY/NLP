...
model = Word2Vec(brown.sents() , size=100, window=5, min_count=1, workers=4)
def fiveFolds(data):
    n=len(data)
    n_split = n//5
    # random.shuffle(data)
...
def Viterbi(test_set,train_set):
    ...
    for k,w in enumerate(test_set):
        ...
        word_k=test_set[k]
        try:
          e_p=emission_df.loc[test_set[k],tags[0]]
          word_k= test_set[k]
        except KeyError:
          try:
            sim_word = model.wv.most_similar(positive=test_set[k])[:][0]
          except:
            sim_word=[]
            word_k = test_set[k]
          for i in sim_word:
            try:
              # sim_words = model.wv.most_similar(positive=test_set[k])
              e_p=emission_df.loc[i,tags[0]]
              word_k=i
              break
            except KeyError:
              continue
        ...
        for tag in tags:
            ...
            # while(1):
            try: 
                e_p=emission_df.loc[word_k,tag]  # e_p=emission_matrix[test_set[k],tag]
                accept = 1 
            except KeyError:
                e_p = 0
            s_p=t_p*e_p
            p.append(s_p)
            p_transition.append(t_p)
        max_p=max(p)
        state_max=tags[p.index(max_p)]
        if(max_p==0):
          max_p = max(p_transition)
          state_max = tags[p_transition.index(max_p)]
        else:
          state_max = tags[p.index(max_p)]
